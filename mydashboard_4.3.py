...
df_copy2 = df[columns_to_display].copy()
df_copy2.columns = [columns_display_names.get(col, col) for col in df_copy2.columns]
#________________________________________________________________________________
...
app.layout = dbc.Container([
    ...
    dbc.Row([
        dcc.Dropdown(
            ...
            options=[{'label': code, 'value': code} for code in df_copy2['Domaine d\'activité'].unique()],
            multi=False,
            #value=df_copy2['Domaine d\'activité'].unique(),
    ...
    dbc.Row([
        dbc.Card([
            ...
            id='mytable',
            columns=[{'name': columns_display_names.get(col, col), 'id': col} for col in df_copy2.columns ], 
...
def update_table(selected_secteur):
    ...
    # return updated_data
    if not selected_secteur:
        data = df_copy2.to_dict('records')
    else:    
        filtered_df = df_copy2[df_copy2['Domaine d\'activité'].astype(str) == str(selected_secteur)]
